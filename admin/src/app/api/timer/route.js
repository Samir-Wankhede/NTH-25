...
export async function GET(request) {
  try {
    ...
    return NextResponse.json(
      { start_time: result.rows[0].start_time, status: result.rows[0].status },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error fetching timer:", error);
...
export async function POST(request) {
  try {
    ...
    await pool.query(query, [status, start_time, end_time]);
...
export async function DELETE(request) {
  try {
    ...
    if (result.rows.length === 0) {
      return NextResponse.json({ message: "No event found to delete" }, { status: 404 });
    }

    const { start_time, end_time } = result.rows[0];

    await endEvent(start_time, end_time);
